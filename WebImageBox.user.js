...
(function () {
  ...
  const btnConfig = [
    {
      title: "左旋转",
      svg: '<path d="M12 2v2a8 8 0 1 0 8 8h-2a6 6 0 1 1-6-6V2z"/>',
    },
    {
      title: "右旋转",
      svg: '<path d="M12 2v2a6 6 0 1 1-6 6H4a8 8 0 1 0 8-8z"/>',
    },
    { title: "放大", svg: '<path d="M15 11h-4V7h-2v4H5v2h4v4h2v-4h4z"/>' },
    { title: "缩小", svg: '<path d="M5 11v2h10v-2z"/>' },
    {
      title: "下载",
      svg: '<path d="M12 16l4-5h-3V4h-2v7H8z"/><path d="M5 18h14v2H5z"/>',
    },
    {
      title: "下载所有",
  ...
  const showImage = (index, direction = 0) => {
    ...
    // 获取该图片的状态，如果没有则初始化
    const state = imgStates.get(imgs[currentIndex].src) || {
      rotation: 0,
      scale: 1,
    };
  ...
  const isSmallOrAvatar = (img) => {
    ...
    const checkString = (str) =>
      keywords.some((k) => (str || "").toLowerCase().includes(k));
    // 检查 img 本身
    if (
      checkString(img.src) ||
      checkString(img.className) ||
      checkString(img.id)
    )
      return true;
    ...
    while (parent) {
      if (
        checkString(parent.href) ||
        checkString(parent.className) ||
        checkString(parent.id)
      )
        return true;
  ...
  const setupImages = () => {
    ...
    pageImgs.forEach((img) => {
      ...
      if (
        !uniqueSrc.has(img.src) &&
        !uniqueName.has(fileName) &&
        !isSmallOrAvatar(img)
      ) {
  ...
  // 控制按钮事件
  const [rotateL, rotateR, zoomIn, zoomOut, download, downloadAll, closeBtn] =
